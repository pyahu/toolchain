...
run_installer "$xdg_home" java cloud ai > "$TEST_ROOT/first-install.out"
...
assert_link "$config_dir/config.ai.toml" "$REPO_DIR/mise.ai.toml"
assert_missing "$config_dir/mise.lock"
assert_missing "$config_dir/mise.java.lock"
assert_missing "$config_dir/bin/kubectl-ctx"
run_installer "$xdg_home" java cloud ai > "$TEST_ROOT/second-install.out"
